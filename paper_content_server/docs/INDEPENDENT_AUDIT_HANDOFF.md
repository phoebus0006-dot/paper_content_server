...
## 6. Implementation Status Notes

- `/admin/api/*` routes do NOT exist (`TARGET_NOT_IMPLEMENTED`)；当前 admin 路由前缀为 `/api/admin/*`。
- Admin LAN/token 配置集中在 `src/config/load-config.js`（`server.js` 仅读取 `APP_CONFIG.admin`）。
- ESP32 NOT compiled / NOT flash-verified。
