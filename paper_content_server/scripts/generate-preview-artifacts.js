/**
 * generate-preview-artifacts.js
 * Captures 1920x1080 visual screenshots of Admin UI into preview-artifacts/admin-ui/
 */
...
const OUT_DIR = path.join(ROOT, 'preview-artifacts', 'admin-ui');
const PORT = 8787;
...
async function main() {
  ...
  const tabs = [
    ...
    { name: 'editor', tabId: 'photo-editor-page' },
    { name: 'publish', tabId: 'publish-page' },
    { name: 'status', tabId: 'status-page' }
  ];

  try {
    console.log('\n--- Capturing 1920x1080 Viewport Screenshots ---');
    ...
    for (const tab of tabs) {
      ...
      const fileName = `${tab.name}-1920x1080.png`;
    ...
    await browser.close();
    if (server) server.kill();
    console.log('\nAll 1920x1080 screenshots saved to:', OUT_DIR);
  }
}
