...
async function run() {
  ...
  // 1. 无 modelPath → classify rejects (NO_CLASSIFIER_MODEL_CONFIGURED)
  ...
  // 2. isSafe 基于 score vs threshold(fail-closed 由 classify reject 保证;
  //    调用方拿到 score=undefined 的分类时 isSafe 返回 false)
  ...
  t('MISSING_MODELPATH_CLASSIFY_REJECTS', classifyErrMissing !== null && classifyErrMissing.message === 'NO_CLASSIFIER_MODEL_CONFIGURED',
  ...
  try {
    ...
    // 9. isSafe 仍基于 score vs threshold(classify 已 reject,但 isSafe 逻辑可独立验证)
