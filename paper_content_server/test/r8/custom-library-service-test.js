...
var store={storeQuarantine:function(f){var q=path.join(tmp,'q.jpg');try{fs.copyFileSync(f,q);}catch(e){}return q;},decodeAndRecompute:function(q){return{fileSize:10,sha256:'abc',mimeType:'image/jpeg'};},moveToAssets:function(q,id){var f=path.join(tmp,id+'.jpg');try{fs.renameSync(q,f);}catch(e){}return f;},cleanup:function(f){try{fs.unlinkSync(f);}catch(e){}}};
...
var safetyGate={isSafe:function(p){return true}};
var ar={create:function(a){return Promise.resolve(a.assetId)}};
var svc=SVC(store,val,dedup,safetyGate,ar,{});
(async function(){var r=await svc.processUpload({filePath:src,originalName:'test.jpg',mimeType:'image/jpeg',fileSize:100,width:800,height:600});
