#!/usr/bin/env node
var path=require('path'),fs=require('fs');var ROOT=path.join(__dirname,'..','..');var ec=0,pass=0,fail=0;function t(n,o,d){console.log((o?'PASS':'FAIL')+' '+n+(d?': '+d:''));if(o)pass++;else{ec=1;fail++}}
...
var cDir=path.join(ROOT,'src','custom-library');var issues=check(cDir,['./','../assets/','path','fs','crypto']);
t('CUSTOM_BOUNDARY',issues.length===0,issues.join(';'));
var allSrc=fs.readdirSync(cDir).filter(function(f){return f.endsWith('.js');}).map(function(f){return fs.readFileSync(path.join(cDir,f),'utf8');}).join('\n');
