...
s('ONE_SHOT_ROUTE',hasOneShotRoute?'IMPLEMENTED':'NOT_IMPLEMENTED',hasOneShotRoute?'POST /api/admin/publish/one-shot route uses assetSelectionService.selectForOneShot() for explicit asset selection; 400 on selection failure':'/api/admin/publish/one-shot does not exist');
...
s('FOCUS_LOCK',hasFocusLock?'IMPLEMENTED':'NOT_IMPLEMENTED',hasFocusLock?'PUT/DELETE /api/admin/focus-lock uses assetSelectionService.selectForFocusLock() for theme/albumId matching; 404 on no match (no schedule fallback)':'No FOCUS_LOCK code in server.js');
