...
function check(dir,allowed){var files=fs.readdirSync(dir).filter(function(f){return f.endsWith('.js');});var issues=[];
...
var lDir=path.join(ROOT,'src','learning');var issues=check(lDir,['./','../assets/']);
t('LEARNING_BOUNDARY',issues.length===0,issues.join(';'));
var allSrc=fs.readdirSync(lDir).filter(function(f){return f.endsWith('.js');}).map(function(f){return fs.readFileSync(path.join(lDir,f),'utf8');}).join('\n');
