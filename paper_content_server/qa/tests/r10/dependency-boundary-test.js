...
var aDir=path.join(ROOT,'src','admin');var issues=check(aDir,['./','../snapshot/','../publication/','../assets/','../safety/']);
t('ADMIN_BOUNDARY',issues.length===0,issues.join(';'));
var allSrc=fs.readdirSync(aDir).filter(function(f){return f.endsWith('.js');}).map(function(f){return fs.readFileSync(path.join(aDir,f),'utf8');}).join('\n');
