...
s('FOCUS_LOCK',hasFocusLock?'IMPLEMENTED_NOT_PRODUCTION_VERIFIED':'NOT_IMPLEMENTED',hasFocusLock?'PUT/DELETE /api/admin/focus-lock uses assetSelectionService.selectForFocusLock() for strict theme/albumId matching (404 on no match, no schedule fallback); override persisted via overridePersistence; restart-validated same as ONE_SHOT; DELETE clears override via overridePersistence.clearOverride(); not yet verified on ESP32 true device':'No FOCUS_LOCK code in server.js');
