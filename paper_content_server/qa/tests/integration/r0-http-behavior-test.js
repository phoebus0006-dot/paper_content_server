...
function spawnSrv(tmpDir, id, envOverrides) {
  ...
  processes.push(child); if(child.pid) pids.push(child.pid); return { child: child, port: env.PORT, base: b, exited: function () { return new Promise(function (r) { child.on('exit', r); }); } };
}
...
test('r0-http-behavior-test.js', async () => { await main(); });
