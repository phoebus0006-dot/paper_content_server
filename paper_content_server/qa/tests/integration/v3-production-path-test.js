...
function spawnSrv(dataDir, instanceId, port, envOverrides) {
  ...
  processes.push(child); if(child.pid) pids.push(child.pid); return { child: child, port: port, base: 'http://127.0.0.1:' + port, exited: function () { return exited; }, stderr: function () { return Buffer.concat(stderrBuf).toString(); } };
}
...
async function main() { const processes = []; const pids = []; let runId = "run_" + Date.now().toString(36); const runDir = path.join(ROOT, "qa", "runtime", runId); fs.mkdirSync(runDir, {recursive: true}); try {
  ...
  } finally { try { fs.writeFileSync(path.join(runDir, "pids.json"), JSON.stringify(pids)); } catch(e){} for(const p of processes) { try { p.kill("SIGKILL"); } catch(e){} } try { fs.rmSync(runDir, {recursive:true, force:true}); } catch(e){} process.exit(ec); }
}



test('v3-production-path-test.js', async () => { await main(); });
